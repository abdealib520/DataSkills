...
class DataEngineer():
    def __init__(self):
        ...
        self.data_languages = ['Python', 'R', 'SQL', 'Java', 'Scala', 'C++', 'JavaScript']
        self.softwares = ['Excel', 'Power BI', 'Tableau', 'SAS', 'Apache Spark', 'Redshift', 'BigQuery', 'Airflow', 'Snowflake']
    def programming(self):
        D = dict()
        for i in self.data_languages:
            D[i] = self.df[i].sum() / len(self.df[i]) * 100
    ...
    def software(self):
        ...
        a = plt.figure(figsize=(12, 7))
        plt.bar(range(len(D)), list(D.values()), align='center',
                color=['#093c5f', '#064e80', '#0b6db0', '#078be6', '#078be6', '#078be6', '#078be6', '#078be6', '#078be6'])
        plt.xticks(range(len(D)), list(D.keys()))
        plt.xlabel('Software')
        plt.ylabel('Percentage')
        return a
